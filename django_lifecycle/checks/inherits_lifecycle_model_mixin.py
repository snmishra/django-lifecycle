...
def get_models_to_check(app_configs: Union[Iterable[AppConfig], None]) -> Iterable[Type[models.Model]]:
...
def model_has_hooked_methods(model: Type[models.Model]) -> bool:
    for attribute_name, attribute in inspect.getmembers(model, predicate=inspect.isfunction):
...
def check_models_with_hooked_methods_inherit_from_lifecycle(
    ...
    for model in get_models_to_check(app_configs):
        if model_has_hooked_methods(model) and not issubclass(model, LifecycleModelMixin):
            yield Error(
